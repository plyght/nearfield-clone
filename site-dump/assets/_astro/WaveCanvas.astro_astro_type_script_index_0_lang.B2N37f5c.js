...
`;
function ae(o, s, e) {
  const n = o.createShader(s);
  if ((o.shaderSource(n, e), o.compileShader(n), !o.getShaderParameter(n, o.COMPILE_STATUS))) {
    const b = o.getShaderInfoLog(n);
    throw (o.deleteShader(n), new Error("Wave shader compile error: " + b));
  }
  return n;
}
const c = (o, s, e) => o + (s - o) * e;
function qe(o, s) {
  const e =
    o.getContext("webgl", {
      antialias: !0,
  ...
  let n;
  try {
    const t = ae(e, e.VERTEX_SHADER, _e),
      a = ae(e, e.FRAGMENT_SHADER, He);
    if (
      ((n = e.createProgram()),
      e.attachShader(n, t),
      e.attachShader(n, a),
      ...
      throw new Error("Wave program link error: " + e.getProgramInfoLog(n));
  } catch (t) {
  ...
  const _ = e.getAttribLocation(n, "aPos");
  (e.enableVertexAttribArray(_), e.vertexAttribPointer(_, 2, e.FLOAT, !1, 0, 0));
  const r = (t) => e.getUniformLocation(n, t),
    ne = r("uRes"),
    re = r("uPhase"),
    se = r("uBase"),
    ie = r("uInk"),
    ...
    we = r("uPhaseOff"),
    Ae = r("uOverscan"),
    P = s.dataset.waveVariant === "settings",
    L = P ? xe : I.base,
    R = P ? Te : B.base;
  (e.uniform3fv(ie, P ? Re : Le), e.uniform1f(ce, We), e.uniform1f(le, 1), e.uniform1f(ue, Ie));
  ...
  function D() {
    ...
    x = Math.min(window.devicePixelRatio || 1, 2);
    const a = Math.max(1, Math.round(t.width * x)),
      i = Math.max(1, Math.round(t.height * x));
    (a === A && i === E) ||
      ((A = a), (E = i), (o.width = a), (o.height = i), e.viewport(0, 0, a, i), F || W(p));
  }
  const v = window.matchMedia("(prefers-reduced-motion: reduce)"),
    ...
    Oe = performance.now();
  function ye() {
    if (v.matches) return 1;
    const t = Math.min(Math.max((performance.now() - Oe - Ee) / Fe, 0), 1);
  ...
  function Y() {
    ...
    for (let f = 0; f < 2; f++) {
      const l = I.waves[f],
        u = B.waves[f];
      ((H[f] = c(l.opacity, u.opacity, t)),
        (q[f] = c(l.lineWidth, u.lineWidth, t) * a * (0.3 + 0.7 * i)),
        (N[f] = c(l.frequency, u.frequency, t)),
        (V[f] = c(l.amplitude, u.amplitude, t) * i),
        (G[f] = c(l.falloff, u.falloff, t)),
        (C[f] = c(l.verticalPosition, u.verticalPosition, t)),
        (U[f] = c(l.phaseOffset, u.phaseOffset, t)),
        (k[f] = c(l.overscanFraction, u.overscanFraction, t) * A));
    }
    ((w[0] = c(L[0], R[0], t)),
      (w[1] = c(L[1], R[1], t)),
      (w[2] = c(L[2], R[2], t)),
  ...
  function K(t) {
    const a = Math.min(Math.max((t - T) / 1e3, 0), 0.1);
    ((T = t), (m += (h - m) * (1 - Math.exp(-a / Be))));
    const i = c(I.speed, B.speed, m);
    ((p += a * i * be), Y(), Math.abs(h - m) < 0.001 && (m = h), (g = requestAnimationFrame(K)));
  }
  function d() {
    F ||
      !S ||
      document.hidden ||
      v.matches ||
      ((F = !0), (T = performance.now()), (g = requestAnimationFrame(K)));
  }
  function O() {
    ((F = !1), g && cancelAnimationFrame(g), (g = 0));
  }
  const X = new ResizeObserver(() => D());
  X.observe(s);
  const z = new IntersectionObserver(
    (t) => {
      ((S = t[0]?.isIntersecting ?? !0), S ? d() : O());
    },
    { threshold: 0 },
  );
  z.observe(s);
  const j = () => (document.hidden ? O() : d());
  document.addEventListener("visibilitychange", j);
  const y = (t) => (t instanceof Element ? t.closest(Me) : null),
    J = () => {
      ((s.dataset.waveHover = "true"), (h = 1), d());
    },
    Q = () => {
      ((s.dataset.waveHover = "false"), (h = 0), d());
    };
  s.dataset.waveHover = "false";
  const Z = (t) => {
      const a = y(t.target);
      if (!a) return;
      const i = t.relatedTarget instanceof Node ? t.relatedTarget : null;
      a.contains(i) || J();
    },
    $ = (t) => {
      const a = y(t.target);
  ...
  (document.addEventListener("pointerover", Z),
    document.addEventListener("pointerout", $),
    document.addEventListener("focusin", ee),
    document.addEventListener("focusout", te));
  const oe = () => {
    v.matches ? (O(), W(p)) : d();
  };
  return (
    v.addEventListener?.("change", oe),
    D(),
    W(0),
    d(),
    function () {
      (O(),
        ...
        document.removeEventListener("pointerover", Z),
        document.removeEventListener("pointerout", $),
        document.removeEventListener("focusin", ee),
        document.removeEventListener("focusout", te),
        delete s.dataset.waveHover,
        v.removeEventListener?.("change", oe));
    }
  );
}
const M = [];
document.querySelectorAll("canvas[data-wave]").forEach((o) => {
  const s = o.closest("[data-wave-host]") ?? o.parentElement;
  s && M.push(qe(o, s));
});
